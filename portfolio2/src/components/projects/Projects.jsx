...
export default function Projects() {
  ...
  return (
    <section className="experience min-h-[100vh] lg:h-[90vh] bg-primary  flex flex-col items-center pt-[4rem] lg:pt-[8rem]">
      <h1 className='text-secondary lg:mb-[4rem] mb-[1.5rem] text-[1.5rem] lg:text-[2rem]  '>Palces I have worked</h1>
     
      <div className="experience-wrapper border-[3px] rounded-xl px-6  border-opacity-[0.1] border-secondary 
       min-h-[400px] pb-[50px] container lg:py-[4rem] mt-[20px]
      ">
       
        {projects.map(item =>
        <div className="main flex">
          <div className="project-list border flex-1 h-10">

          </div>
         <div key={item.id} className="project-card h-[fit] relative
         flex gap-[40px] justify-end bg-secondary  border-[yellow] border-4
         ">
            
          <div  className="desc text-left">
  <h1 className='text-[white] text-[1.8rem] '>{item.title}</h1>
  <p className='text-[white]'>{item.desc}</p>
  <p className='text-[white]'>{item.tools}</p>
  </div>

    <div className="img border-2 w-[400px]">
            <img className='h-[300px] w-[100%]' src={item.pix} alt="" />
            </div>
