  ...
  return (
    ...
    <nav className={shadow ? ' bg-primary sticky left-0 right-0 shadow-xl z-[2000] top-0 NavShadow  ' : ' bg-primary  sticky left-0 right-0  z-[2000]  top-0 NavShadow'} >
      ...
      <div className='navContainer container flex items-center  justify-between py-[20px] md:py-[30px] 3xl:py-[45px] '>
        <motion.div className="logo cursor-pointer relative z-[90] "
          initial={{ x: -200, opacity: 0.2 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 1 }}>
          <Link to='hero' spy={true} smooth={true} offset={0} duration={300} onClick={() => setToggle(true)}><img className='w-[40px] lg:w-[50px] ' src={logo} alt="logo" /></Link>

        </motion.div>
        <div className="links z-40 ">
          <motion.ul className={toggle ? 'hidden  md:bg-opacity-0 flex-col items-start  gap-[20px] absolute top-0 left-0 right-0 pt-[128px] pb-[44px]  md:flex md:gap-[30px] md:flex-row md:relative md:p-0 ' : '  flex flex-col items-start  bg-[white]  md:bg-[inherit] absolute top-0 left-0 right-0 pt-[128px] pb-[44px]  md:flex md:gap-[30px] md:flex-row md:relative md:p-0'} onClick={() => setToggle(true)}
        ...
        <div className="icon space-x-8 flex items-center z-[90] md:hidden ">
          ...
          <motion.div className="toggles md:hidden relative z-[290]" onClick={toggleMenu}
            initial={{ x: 400, opacity: 0.2 }}
            animate={{ x: 0, opacity: 1 }}
